...
- **이벤트 기반 연동:**
  - `order.start` (Kafka): order-service로부터 생산 시작 이벤트 수신
  - `inventory.deduct` (Kafka): 조립 단계(ASSEMBLY_A/B) 진입 시 부품 차감 요청
  - `shipping.ready` (Kafka): QC 완료 시 배송 등록 요청
  - `line.fault` (Kafka): 장애 발생 시 실시간 모니터링 알림
- **배송 연동:** QC 완료 시 `shipping.ready` Kafka 이벤트로 배송 등록 (비동기)
...
|:---|:---:|:---|:---|
| **Order Service** | Async (Kafka) | `order.start` (Sub) | 생산 시작 신호 및 주문 정보 수신<br>Payload: `{ orderId, orderNumber, carModel, carColor, totalQuantity }` |
...
| **Inventory Service** | Async (Kafka) | `inventory.deduct` (Pub) | 조립 단계(ASSEMBLY_A/B) 진입 시 부품 차감 요청<br>Payload: `{ vehicleId, station, parts: [{partId, quantity}] }` |
| **Inventory Service** | Sync (Feign) | `POST /inventory/error/stock/{partId}` | 재고 부족 상황 강제 연출 (시연용) |
| **Shipping Service** | Async (Kafka) | `shipping.ready` (Pub) | QC 완료 시 배송 등록 요청<br>Payload: `{ vin, orderId: Integer }` |
...
order-service와 연동된 전체 플로우를 포스트맨으로 테스트합니다.

#### 1단계 — 시뮬레이터 시작
```
POST http://localhost:8086/simulator/executions
```

#### 2단계 — 주문 생성 (order-service)
```
...
#### 3단계 — 생산 시작 (Kafka 이벤트 발행)
```
POST http://localhost:8082/orders/1/start
```
order-service가 `order.start` 토픽으로 이벤트를 발행하고, simulator가 수신하여 차량을 투입합니다.

#### 4단계 — 투입 확인
```
GET http://localhost:8086/simulator/logs
```
`[투입] 1-1 — vehicleId 매핑 실패...` 또는 `[투입] 1-1 → vehicleId=...` 로그가 보이면 정상
> vehicleId 매핑은 line-service에 해당 orderId 차량이 등록되어 있을 때 성공합니다.

#### 5단계 — 차량 강제 이동 (QC까지 6번, 시연용)
```
POST http://localhost:8086/simulator/move/1-1   ← 6번 반복
```
공정 순서: `BODY_A → BODY_B → PAINTING → ASSEMBLY_A → ASSEMBLY_B → QC → 완료`

- `ASSEMBLY_A`, `ASSEMBLY_B` 진입 시 → `inventory.deduct` Kafka 이벤트 발행 (재고 차감)
- `QC` 완료 시 → `shipping.ready` Kafka 이벤트 발행 (배송 등록)

#### 6단계 — inventory-service 연동 확인 (재고 차감)
```
GET http://localhost:8084/inventory/parts
```
`partId` 1, 2, 3의 재고가 각 1씩 차감되었으면 정상

> 차감 부품: `partId 1 (엔진오일)`, `2 (에어필터)`, `3 (브레이크패드)`, 각 수량 1

#### 7단계 — shipping-service 연동 확인 (배송 등록)
```
GET http://localhost:8085/shippings
```
새 배송 레코드(SHIP-005 등)가 생성되었으면 전체 플로우 완료
...
| Eureka | `http://localhost:8761/eureka/` | 비활성화 (직접 URL로 Feign 호출) |
| Line Service | `http://localhost:8083` | `http://line-service:8083` |
| Inventory Service | `http://localhost:8084` | `http://inventory-service:8084` |
| Shipping Service | `http://localhost:8085` | `http://shipping-service:8085` |
...
- **inventory-service 연동:** `ASSEMBLY_A`, `ASSEMBLY_B` 두 단계 모두 재고 차감 이벤트를 발행합니다. Payload 형식은 `{ vehicleId, station, parts: [{partId, quantity}] }` 이며 inventory-service 스펙에 맞춰 구현되어 있습니다.
- **shipping-service 연동:** 기존 Feign 동기 방식에서 Kafka 비동기(`shipping.ready` 토픽)로 변경되었습니다. Payload의 `orderId`는 Integer 타입으로 발행하며, shipping-service의 `ShippingRequestDTO` 스펙과 일치합니다.
- **order-service 연동:** `order.start` 토픽 구독. order-service의 `OrderStartedEvent` 스펙(`orderId: Long`, `carModel`, `totalQuantity`)에 맞춰 simulator `OrderStartedEvent` DTO를 정렬했습니다.
