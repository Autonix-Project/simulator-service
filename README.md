...
`order-service` 없이 Kafka CLI와 REST API를 이용해 전체 흐름을 테스트할 수 있습니다.
...
```
프롬프트(`>`)가 뜨면 아래 JSON 입력 후 Enter, `Ctrl+C`로 종료:
```json
{"orderId":"1","modelName":"SEDAN","quantity":1}
```
...
```
`[투입] 1-1 → vehicleId=...` 로그가 보이면 정상

#### 4단계 — 차량 강제 이동 (QC까지 6번)

`order-service` 미연동 상태에서 빠른 테스트를 위해 강제 이동 API를 사용합니다.
...
- `ASSEMBLY_A`, `ASSEMBLY_B` 진입 시 → `inventory.deduct` Kafka 이벤트 발행 (부품 차감)
- `QC` 완료 시 → `shipping.ready` Kafka 이벤트 발행 (배송 등록)

#### 5단계 — inventory-service 연동 확인 (재고 차감)
```
GET http://localhost:8084/inventory/parts
```
`partId` 1, 2, 3의 재고가 각 1씩 차감되었으면 정상

> 테스트 DB 기준 차감 부품: `partId 1 (엔진오일)`, `2 (에어필터)`, `3 (브레이크패드)`, 각 수량 1

#### 6단계 — shipping-service 연동 확인 (배송 등록)
```
GET http://localhost:8085/shippings
```
`orderId: 1`인 새 배송 레코드(SHIP-004 등)가 생성되었으면 정상
